...
enum CornerType {
    ...
    case middle
    case bottom
    case excludeTopRightCorner
    case excludeTopLeftCorner
    case excludeBottomLeftCorner
    case excludeBottomRightCorner
    case searchViewTop
    case none
    case messageInfoHeaderAllCorners
    case messageInfoHeaderTopCorners
    case userInfoCellBottomCorners
}
...
extension UIView {
    ...
    func updateContainerLayerFor(cornerType: CornerType) {
        var brazierPath: UIBezierPath!
        switch cornerType {
        case .complete:
            brazierPath =  UIBezierPath(roundedRect: self.bounds,
                                        byRoundingCorners: [.allCorners],
                                        cornerRadii: CGSize(width: 15.0, height: 0.0))
        case .top:
            brazierPath =  UIBezierPath(roundedRect: self.bounds,
                                        byRoundingCorners: [.topLeft, .topRight],
                                        cornerRadii: CGSize(width: 12.0, height: 0.0))
        case .searchViewTop:
            brazierPath =  UIBezierPath(roundedRect: self.bounds,
                                        byRoundingCorners: [.topLeft, .topRight],
        ...
        case .bottom:
            brazierPath =  UIBezierPath(roundedRect: self.bounds,
                                        ...
                                        cornerRadii: CGSize(width: 12.0, height: 0.0))
        case .none, .excludeTopRightCorner, .excludeTopLeftCorner:
            brazierPath =  UIBezierPath(roundedRect: self.bounds,
                                        byRoundingCorners: [],
        ...
        case .excludeBottomLeftCorner:
            ...
            brazierPath =  UIBezierPath(roundedRect: self.bounds,
                                        byRoundingCorners: [.topLeft, .bottomRight, .topRight],
                                        cornerRadii: CGSize(width: 15.0, height: 0.0))
    ...
    func roundPerticularCorners(_ corners: UIRectCorner, radius: CGFloat, maskedCorner: CACornerMask, width: CGFloat = 0.0 ) {
    ...
    func drawShadow(shadowColor:UIColor = #colorLiteral(red: 0.2549019754, green: 0.2745098174, blue: 0.3019607961, alpha: 0.798828125), shadowOpacity:Float = 0.14, shadowPath:UIBezierPath? = nil, shadowRadius:Float = 5, cornerRadius:Float = 6.0, offset: CGSize = CGSize(width: -1, height: 1), scale: Bool = true) {
...
extension UIView {
    ...
    Convert UIView to UIImage some other style **/
    ...
    func asImage() -> UIImage {
        ...
        UIGraphicsEndImageContext()
        if let getImg = outputImage{
            return getImg
        }
        return UIImage()
    }
        
    func asOptionalImage(with size: CGSize) -> UIImage? {
        return UIGraphicsImageRenderer(size: size).image { _ in
            drawHierarchy(in: CGRect(origin: .zero, size: size), afterScreenUpdates: true)
        }
    }
    
    func blurrEffect(blurrStyle: UIBlurEffect.Style){
        
        let blurEffect = UIBlurEffect(style: blurrStyle)
        let blurredEffectViews = self.subviews.filter{$0 is UIVisualEffectView}
        if blurredEffectViews.isEmpty{
    ...
    func imageSnapshotCroppedToFrame(frame: CGRect?) -> UIImage {
        ...
        var image: UIImage = UIGraphicsGetImageFromCurrentImageContext() ?? UIImage()
        UIGraphicsEndImageContext()
...
extension UIView {
    ...
    func rippleAnimation() {
        ...
        //        impliesAnimation.duration = duration * 2.0
        impliesAnimation.values = [1.0 ,1.1, 0.95, 1.1, 0.97, 1.01, 1.0]
        impliesAnimation.duration = 0.3 * 2.0
    ...
    func addrippleEffect(){
        ...
        if let layer = self.layer.sublayers?.first(where: {$0.name != "Ripple"}) as? CAShapeLayer{
            rippleShape = layer
        }else{
